...
def compute_fid(fake_arr, gt_dir, device,
    ...
    patch_fid=False):
    from src.eval.coco_eval.cleanfid import fid
...
def compute_clip_score(
    images, captions, clip_model="ViT-G/14", device="cuda", how_many=30000):
    ...
    if clip_model == "ViT-B/32":
        ...
        import open_clip
        clip, _, clip_preprocessor = open_clip.create_model_and_transforms("ViT-g-14", pretrained="/data3/MUSES/model/CLIP-G-14/open_clip_pytorch_model.bin")
        clip = clip.to(device)
        clip = clip.eval()
        clip = clip.float()
    else:
