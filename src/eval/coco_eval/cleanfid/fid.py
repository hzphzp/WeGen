...
import zipfile
from src.eval.coco_eval import cleanfid
from src.eval.coco_eval.cleanfid.utils import *
from src.eval.coco_eval.cleanfid.features import build_feature_extractor, get_reference_statistics
from src.eval.coco_eval.cleanfid.resize import *
...
def get_array_features(l_array, model=None, num_workers=12,
    ...
    for batch in pbar:
        l_feats.append(get_batch_features(batch, model, device))
    np_feats = np.concatenate(l_feats)
    print("generated:", np_feats.shape)
    return np_feats
...
def make_custom_stats(name, fdir, num=None, mode="clean", model_name="inception_v3",
    ...
    if model_name=="inception_v3":
        ...
        from src.eval.coco_eval.cleanfid.clip_features import CLIP_fx, img_preprocess_clip
