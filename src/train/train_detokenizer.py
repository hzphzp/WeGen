...
class TrainingArguments:
    ...
    warmup_resampler_steps: int = field(default=0, metadata={"help": "Number of updates steps before the resampler warmuped then unfreeze the unet."})
    ...
    enable_gradient_checkpointing: bool = field(default=False, metadata={"help": "Enable gradient checkpointing."})
...
def train():
    ...
    if cfg_path.diffusion_specific_unet_path is not None:
        unet = UNet2DConditionModel.from_pretrained(cfg_path.diffusion_specific_unet_path)
    else:
    ...
    if global_step < args.warmup_resampler_steps:
        # freeze the unet
        adapter.freeze_unet()
    ...
    if accelerator.is_main_process:
        with open('./trained_params.txt', 'w') as f:
            for name, param in adapter.named_parameters():
                if param.requires_grad:
                    f.write(name + '\n')
    # # write the optimizer parameters name to file
    # if accelerator.is_main_process:
    #     with open('./optimizer_params.txt', 'w') as f:
    #         for name, param in optimizer['param_groups'][0].items():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(adapter):
                ...
                images_aug = batch['images_aug'].to(accelerator.device, dtype=weight_dtype) if batch['images_aug'] is not None else None
                ...
                with torch.no_grad():
                    if images is not None:
                        ...
                        image_embeds = None
                output = adapter.train_pipeline(image_embeds, images_aug, accelerator.device, weight_dtype)
            ...
            if accelerator.sync_gradients and accelerator.is_main_process:
                ...
                if global_step % args.training_sample_steps == 0:
                    log_sample_training(diffusion_model_path, adapter, unet, vae, accelerator, weight_dtype, global_step, args.output_dir, batch)
            accelerator.wait_for_everyone()
...
def log_sample_training(pretrained_model_name_or_path, adapter, unet, vae, accelerator, weight_dtype, global_step=1, local_path=None, batch=None):
    ...
    images_aug = batch['images_aug'].to(accelerator.device, dtype=weight_dtype)
    image = images[0].unsqueeze(0)
    image_aug = images_aug[0].add(1).div(2)
    from torchvision.transforms import ToPILImage
    image_aug = ToPILImage()(image_aug)
    ...
    image_aug.save(f"{local_path}/results/train_sample_global_step_{global_step}_input_image.png")
    result.save(f"{local_path}/results/train_sample_global_step_{global_step}_recon_image.png")
