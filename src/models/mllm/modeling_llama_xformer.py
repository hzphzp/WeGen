    ...
    def forward(
        ...
        if self.training:
            ...
            attn_output = F.scaled_dot_product_attention(
                ...
                key_states,
                value_states,
                is_causal=False if attention_mask.sum() == 0 else True,
            )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # check if nan or inf in hidden_states
        if torch.isnan(hidden_states).any():
            print(f'nan in hidden_states')
        elif torch.isinf(hidden_states).any():
            print(f'inf in hidden_states')

        logits = self.lm_head(hidden_states)

        # check if nan or inf in logits
        ...
        if labels is not None:
            ...
            # loss = loss_fct(shift_logits.float(), shift_labels).type_as(shift_logits)

            # check if nan or inf in loss
