...
class ContinuousLVLMJointLLMSDAuto(ContinuousLVLM):
    ...
    def __init__(self, adapter, *args, **kwargs) -> None:
        ...
        self.rec_loss_attr = kwargs.pop('rec_loss_attr', ['mse_loss'])
        self.rec_loss_attr_weight = kwargs.pop('rec_loss_attr_weight', [1.0])
    ...
    def forward(self, input_ids, attention_mask, labels, image_embeds, embeds_gen_mask, embeds_cmp_mask, ids_gen_mask,
        ...
        if image_embeds is not None:
            ...
            image_embeds_gen = image_embeds[embeds_gen_mask]
            if patch_positions is not None:
                if image_embeds_cmp.shape[0] > 0:
                    patch_positions = patch_positions[embeds_cmp_mask]
                else:
                    patch_positions = patch_positions[embeds_gen_mask]
